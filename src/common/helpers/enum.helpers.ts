...
export function enumValuesUpper<T extends Record<string, string>>(
  obj: T,
): UnionToArray<EnumValues<T[keyof T]>> {
  return Object.values(obj).map((value) => value.toUpperCase()) as UnionToArray<
    EnumValues<T[keyof T]>
  >;
}
